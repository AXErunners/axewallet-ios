...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        topOverscrollView.backgroundColor = [UIColor dw_axeNavigationBlueColor];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (transaction) {
        ...
        [self.delegate homeViewShowAxePayRegistrationFlow:self];
    }
}
...
- (void)registrationErrorRetryAction {
    if ([self.model.axePayModel canRetry]) {
        [self.model.axePayModel retry];
    }
    else {
        [self.delegate homeViewShowAxePayRegistrationFlow:self];
    }
