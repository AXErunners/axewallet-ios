...
- (NSAttributedString *)axeAmountStringFrom:(id<DWTransactionListDataItem>)transactionData
                                        font:(UIFont *)font {
    const uint64_t axeAmount = transactionData.axeAmount;
    UIColor *tintColor = transactionData.axeAmountTintColor;

    NSNumberFormatter *numberFormatter = [DSPriceManager sharedInstance].axeFormat;

    NSNumber *number = [(id)[NSDecimalNumber numberWithLongLong:axeAmount]
    ...
    return [NSAttributedString dw_axeAttributedStringForFormattedAmount:string tintColor:tintColor font:font];
}
