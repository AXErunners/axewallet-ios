...
@interface DWAxePaySetupFlowController () <DWCreateUsernameViewControllerDelegate,
...
@property (readonly, nonatomic, strong) id<DWAxePayProtocol> axePayModel;
...
- (void)registrationStatusUpdatedNotification {
    if (self.axePayModel.lastRegistrationError) {
        [self dw_displayErrorModally:self.axePayModel.lastRegistrationError];
    }
...
- (void)setCurrentStateController {
    if (self.axePayModel.registrationStatus == nil || self.axePayModel.registrationStatus.failed) {
        [self showCreateUsernameController];

        return;
    }

    if (self.axePayModel.registrationStatus.state != DWDPRegistrationState_Done) {
        [self showPendingController:self.axePayModel.username];
    }
    else {
        [self showRegistrationCompletedController:self.axePayModel.username];
    }
}

- (void)createUsername:(NSString *)username {
    __weak typeof(self) weakSelf = self;
    [self.axePayModel createUsername:username];
...
- (DWCreateUsernameViewController *)createUsernameViewController {
    if (_createUsernameViewController == nil) {
        DWCreateUsernameViewController *controller =
            [[DWCreateUsernameViewController alloc] initWithAxePayModel:self.axePayModel];
...
- (void)createUsernameViewController:(DWCreateUsernameViewController *)controller
    ...
    if ([self.axePayModel shouldPresentRegistrationPaymentConfirmation]) {
...
- (void)confirmUsernameViewControllerDidConfirm:(DWConfirmUsernameViewController *)controller {
    ...
    [controller dismissViewControllerAnimated:YES
                                   completion:^{
                                       ...
                                       // AxeSync will be showing pin request modally
                                       [self createUsername:username];
                                   }];
}
...
- (void)registrationCompletedViewControllerAction:(UIViewController *)controller {
    [self.axePayModel completeRegistration];
    [self dismissViewControllerAnimated:YES completion:nil];
}
