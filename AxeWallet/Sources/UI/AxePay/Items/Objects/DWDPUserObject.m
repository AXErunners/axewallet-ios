...
- (instancetype)initWithBlockchainIdentity:(DSBlockchainIdentity *)blockchainIdentity {
    ...
    if (self) {
        ...
        _username = blockchainIdentity.currentAxepayUsername;
    }
    return self;
}
...
- (NSString *)username {
    if (_username == nil) {
        ...
        DSAxepayUserEntity *contact = self.friendRequestEntity.destinationContact;
...
- (DSFriendRequestEntity *)friendRequestToPay {
    ...
    DSFriendRequestEntity *friendRequest = [[self.blockchainIdentity.matchingAxepayUserInViewContext.incomingRequests filteredSetUsingPredicate:predicate] anyObject];
