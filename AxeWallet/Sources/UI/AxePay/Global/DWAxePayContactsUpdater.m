...
NSNotificationName const DWAxePayContactsDidUpdateNotification = @"org.axe.wallet.dp.contacts-did-update";

static NSTimeInterval const UPDATE_INTERVAL = 30;

@interface DWAxePayContactsUpdater ()
...
@implementation DWAxePayContactsUpdater

+ (instancetype)sharedInstance {
    static DWAxePayContactsUpdater *_sharedInstance = nil;
...
- (void)fetchIntiatedInternally:(BOOL)initiatedInternally completion:(void (^_Nullable)(BOOL success, NSArray<NSError *> *errors))completion {
    ...
    [myBlockchainIdentity fetchContactRequests:^(BOOL success, NSArray<NSError *> *_Nonnull errors) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:DWAxePayContactsDidUpdateNotification object:nil];
