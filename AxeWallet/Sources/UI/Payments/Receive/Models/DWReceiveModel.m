...
- (instancetype)initWithAmount:(uint64_t)amount {
    ...
    if (self) {
        _updateQueue = dispatch_queue_create("org.axe.wallet.DWReceiveModel.queue", DISPATCH_QUEUE_SERIAL);
...
- (nullable NSString *)requestAmountReceivedInfoIfReceived {
    ...
    uint64_t total = 0;
    const uint64_t fuzz = [priceManager amountForLocalCurrencyString:[priceManager localCurrencyStringForAxeAmount:1]] * 2;
    ...
    for (DSTransaction *tx in wallet.allTransactions) {
        ...
        if (total + fuzz >= request.amount) {
            ...
            NSString *info = [NSString stringWithFormat:NSLocalizedString(@"Received %@ (%@)", nil),
                                                        [priceManager stringForAxeAmount:self.amount],
                                                        [priceManager localCurrencyStringForAxeAmount:self.amount]];
...
- (void)updateReceivingInfo {
    dispatch_async(self.updateQueue, ^{
        ...
        paymentRequest.axepayUsername = [DWGlobalOptions sharedInstance].axepayUsername;
        ...
        if (paymentRequest && paymentRequest.isValidAsNonAxepayPaymentRequest && rawQRImageData) {
            if (!hasAmount) {
