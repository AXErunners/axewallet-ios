...
- (void)processPaymentInput:(DWPaymentInput *)paymentInput {
    ...
    // re-build input if it's AxePay-compatible
    NSString *requestUsername = paymentInput.request.axepayUsername;
    if (requestUsername) {
        ...
        if (myBlockchainIdentity) {
            ...
            DSAxepayUserEntity *axepayUserEntity = [myBlockchainIdentity matchingAxepayUserInContext:context];
            DSBlockchainIdentity *requestIdentity = nil;
            for (DSFriendRequestEntity *friendRequest in axepayUserEntity.incomingRequests) {
                if ([[friendRequest.sourceContact.associatedBlockchainIdentity.axepayUsername stringValue] isEqualToString:requestUsername]) {
...
- (void)confirmRequest:(DSPaymentRequest *)request {
    ...
    if (!request.isValidAsNonAxepayPaymentRequest) {
        if ([request.paymentAddress isValidAxePrivateKeyOnChain:chain] ||
            [request.paymentAddress isValidAxeBIP38Key]) {
            [self confirmSweep:request];
        }
        else {
            // Currently, only errors from AxeSync are handled.
            // TODO: provide an error (app-specific domain)
            [self failedWithError:nil title:NSLocalizedString(@"Not a valid Axe address", nil) message:nil];
        }
    }
        ...
        [DSPaymentRequest
            ...
            completion:^(DSPaymentProtocolRequest *protocolRequest, NSError *error) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    if (error && !([request.paymentAddress isValidAxeAddressOnChain:chain])) {
...
- (void)confirmSweep:(DSPaymentRequest *)request {
    ...
    DSChain *chain = [DWEnvironment sharedInstance].currentChain;
    const BOOL valid = [privateKey isValidAxePrivateKeyOnChain:chain] || [privateKey isValidAxeBIP38Key];
        ...
        dispatch_group_notify(dispatchGroup, dispatch_get_main_queue(), ^{
            if (errors.count > 0) {
                ...
                [self failedWithError:errors.firstObject
                                title:NSLocalizedString(@"Couldn't transmit payment to Axe network", nil)
                              message:description];
            }
...
- (void)handleSweepResultTx:(nullable DSTransaction *)tx
    ...
    if (error) {
        ...
        NSString *format =
            NSLocalizedString(@"Send %@ (%@) from this private key into your wallet? The Axe network will receive a fee of %@ (%@).", nil);
        NSString *message = [NSString stringWithFormat:format,
                                                       [priceManager stringForAxeAmount:amount],
                                                       [priceManager localCurrencyStringForAxeAmount:amount],
                                                       [priceManager stringForAxeAmount:fee],
