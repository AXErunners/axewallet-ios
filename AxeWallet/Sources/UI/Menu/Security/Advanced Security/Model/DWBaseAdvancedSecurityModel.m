...
- (NSArray<NSNumber *> *)spendingConfirmationValues {
    if (!_spendingConfirmationValues) {
        // Axe values: 0 / 0.1 / 0.5 / 1 / 5
        _spendingConfirmationValues = @[ @(0), @(HAKS / 10), @(HAKS / 2), @(HAKS), @(HAKS * 5) ];
    }
...
- (NSAttributedString *)spendingConfirmationString:(NSNumber *)number
    ...
    return [NSAttributedString dw_axeAttributedStringForAmount:value tintColor:color font:font];
}
