...
- (BOOL)amountIsValidForProceeding {
    ...
    return [self isSufficientAxeForInput];
}
...
- (void)selectAllFundsWithPreparationBlock:(void (^)(void))preparationBlock {
    ...
    uint64_t allAvailableFunds = [self.card.available decimalNumberByMultiplyingBy:haks].longLongValue;
...
- (void)updateCurrentAmount {
    ...
    if ([self isSufficientAxeForInput]) {
        self.descriptionModel = self.availableDescriptionModel;
    }
    else {
...
- (void)createTransactionForAmount:(NSString *)amount feeWasDeductedFromAmount:(BOOL)feeWasDeductedFromAmount otpToken:(nullable NSString *)otpToken {
    ...
    self.createTransactionCancellationToken = [client
        createTransactionForAxeCard:self.card
                              amount:amount
...
- (BOOL)isSufficientAxeForInput {
    NSDecimalNumber *decimalAxe = [self decimalAmountNumber];
    if ([decimalAxe compare:self.card.available] == NSOrderedDescending) {
        return NO;
    }
...
- (NSDecimalNumber *)decimalAmountNumber {
    NSDecimalNumber *haks = (NSDecimalNumber *)[NSDecimalNumber numberWithLongLong:HAKS];
    NSDecimalNumber *axe = (NSDecimalNumber *)[NSDecimalNumber numberWithLongLong:self.amount.plainAmount];
    NSDecimalNumber *result = [axe decimalNumberByDividingBy:haks];
...
- (NSAttributedString *)availableAttributedString {
    ...
    NSAttributedString *available =
        [NSAttributedString dw_axeAttributedStringForFormattedAmount:availableRaw
                                                            tintColor:color
                                                                 font:availableFont];
    ...
    NSAttributedString *suffix =
        [[NSAttributedString alloc] initWithString:NSLocalizedString(@"available", @"lowercase, ex. 4 Axe available")
                                        attributes:attributes];
