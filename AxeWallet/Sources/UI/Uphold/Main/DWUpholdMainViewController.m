...
- (void)viewDidLoad {
    ...
    [self.buyButton setTitle:NSLocalizedString(@"Buy Axe", nil) forState:UIControlStateNormal];
                  ...
                  with:^(typeof(self) self, NSNumber *value) {
                      switch (self.model.state) {
                          case DWUpholdMainModelState_Loading: {
                              self.titleLabel.text = NSLocalizedString(@"Your Uphold account Axe balance is", nil);
                          ...
                          case DWUpholdMainModelState_Done: {
                              ...
                              self.balanceLabel.attributedText = [self.model availableAxeString];
                              self.retryButton.hidden = YES;
...
- (IBAction)transferButtonAction:(id)sender {
    DWUpholdTransferViewController *controller = [[DWUpholdTransferViewController alloc] initWithCard:self.model.axeCard];
    controller.delegate = self;
    [self.navigationController pushViewController:controller animated:YES];
}

- (IBAction)buyButtonAction:(id)sender {
    NSURL *url = [self.model buyAxeURL];
    if (!url) {
        return;
    }
