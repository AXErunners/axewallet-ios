...
+ (BOOL)canHandleURL:(NSURL *)url {
    ...
    return [url.scheme isEqual:@"axe"] || [url.scheme isEqual:@"axewallet"] || [url.scheme isEqual:@"pay"];
}
...
+ (nullable DWURLAction *)actionForURL:(NSURL *)url {
    ...
    else if ([url.scheme isEqual:@"axe"] || [url.scheme isEqual:@"pay"]) {
        DWURLPayAction *action = [[DWURLPayAction alloc] init];
...
+ (nullable NSURL *)paymentURLWithParams:(NSDictionary<NSString *, NSString *> *)params {
    ...
    NSString *componentsString = [NSString stringWithFormat:@"axe:%@", mutableParams[@"pay"]];
