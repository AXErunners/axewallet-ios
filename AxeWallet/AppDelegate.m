...
#import <AxeSync/UIWindow+DSUtils.h>
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(nullable NSDictionary *)launchOptions {
    ...
    [CLMCloudInAppMessaging setupWithCloudKitContainerIdentifier:@"iCloud.org.axe.axewallet"];
    ...
    [[AxeSync sharedSyncController] registerBackgroundFetchOnce];
    ...
    if (migrationManager.shouldMigrate) {
        ...
        // otherwise, `startExchangeRateFetching` will be performed within AxeSync initialization process
...
- (void)applicationDidEnterBackground:(UIApplication *)application {
    ...
    // Schedule background fetch if the wallet (AxeSync) had been started
    DWDataMigrationManager *migrationManager = [DWDataMigrationManager sharedInstance];
    if (!migrationManager.shouldMigrate) {
        [[AxeSync sharedSyncController] scheduleBackgroundFetch];
    }
}
...
performFetchWithCompletionHandler:(void (^)(UIBackgroundFetchResult))completionHandler {
    [[AxeSync sharedSyncController] performFetchWithCompletionHandler:completionHandler];
}
...
- (BOOL)application:(UIApplication *)application
    ...
    if (![DWURLParser canHandleURL:url]) {
        UIAlertController * alert = [UIAlertController
                                     alertControllerWithTitle:NSLocalizedString(@"Not a Axe URL", nil)
...
- (void)setupAxeWalletComponentsWithOptions:(NSDictionary *)launchOptions {
    ...
    [[AxeSync sharedSyncController] setupAxeSyncOnce];
    
    [DWEnvironment sharedInstance]; //starts up the environment, this is needed here
    
#if FRESH_INSTALL
    // TODO: fix. Disabled due to crashing :(
//    [[AxeSync sharedSyncController] wipeBlockchainDataForChain:[DWEnvironment sharedInstance].currentChain];
#endif /* FRESH_INSTALL */
