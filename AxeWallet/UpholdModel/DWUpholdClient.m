...
- (void)getCards:(void (^)(DWUpholdCardObject *_Nullable axeCard, NSArray<DWUpholdCardObject *> *fiatCards))completion {
    NSParameterAssert(self.accessToken);

    __weak typeof(self) weakSelf = self;
    [DWUpholdAPIProvider getCardsAccessToken:self.accessToken completion:^(BOOL success, DWUpholdAPIProviderResponseStatusCode statusCode, DWUpholdCardObject *_Nullable axeCard, NSArray<DWUpholdCardObject *> *fiatCards) {
        __strong typeof(weakSelf) strongSelf = weakSelf;
        if (!strongSelf) {
            return;
        }
        ...
        if (success) {
            if (axeCard) {
                if (!axeCard.address) {
                    [strongSelf createAxeCardAddress:axeCard completion:^(DWUpholdCardObject *_Nullable card) {
                        if (completion) {
                    ...
                    if (completion) {
                        completion(axeCard, fiatCards);
                    }
                }
            }
            else {
                [strongSelf createAxeCard:^(DWUpholdCardObject *_Nullable card) {
                    if (completion) {
                        completion(card, fiatCards);
                    }
                }];
...
- (DWUpholdCancellationToken)createTransactionForAxeCard:(DWUpholdCardObject *)card
    ...
    __weak typeof(self) weakSelf = self;
    return [DWUpholdAPIProvider createTransactionForAxeCard:card amount:amount address:address accessToken:self.accessToken otpToken:otpToken completion:^(BOOL success, DWUpholdAPIProviderResponseStatusCode statusCode, DWUpholdTransactionObject *_Nullable transaction) {
        __strong typeof(weakSelf) strongSelf = weakSelf;
        if (!strongSelf) {
            return;
        }
...
- (DWUpholdCancellationToken)createBuyTransactionForAxeCard:(DWUpholdCardObject *)card
    ...
    __weak typeof(self) weakSelf = self;
    return [DWUpholdAPIProvider createBuyTransactionForAxeCard:card account:account amount:amount securityCode:securityCode accessToken:self.accessToken otpToken:otpToken completion:^(BOOL success, DWUpholdAPIProviderResponseStatusCode statusCode, DWUpholdTransactionObject *_Nullable transaction) {
        __strong typeof(weakSelf) strongSelf = weakSelf;
        if (!strongSelf) {
            return;
        }
...
- (nullable NSURL *)buyAxeURLForCard:(DWUpholdCardObject *)card {
    if (!card.identifier) {
        return nil;
    }
...
- (void)createAxeCard:(void (^)(DWUpholdCardObject *_Nullable card))completion {
    NSParameterAssert(self.accessToken);

    __weak typeof(self) weakSelf = self;
    [DWUpholdAPIProvider createAxeCardAccessToken:self.accessToken completion:^(BOOL success, DWUpholdAPIProviderResponseStatusCode statusCode, DWUpholdCardObject *_Nullable card) {
        __strong typeof(weakSelf) strongSelf = weakSelf;
        if (!strongSelf) {
            return;
        }
        ...
        if (success && card) {
            [strongSelf createAxeCardAddress:card completion:completion];
        }
        else {
            if (completion) {
                completion(nil);
...
- (void)createAxeCardAddress:(DWUpholdCardObject *)card completion:(void (^)(DWUpholdCardObject *_Nullable card))completion {
    NSParameterAssert(self.accessToken);
    NSParameterAssert(card);
    NSAssert(!card.address, @"Card has address already");

    __weak typeof(self) weakSelf = self;
    [DWUpholdAPIProvider createAddressForAxeCard:card accessToken:self.accessToken completion:^(BOOL success, DWUpholdAPIProviderResponseStatusCode statusCode, DWUpholdCardObject *_Nullable card) {
        __strong typeof(weakSelf) strongSelf = weakSelf;
        if (!strongSelf) {
            return;
        }
